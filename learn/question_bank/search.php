...
//Search for questions
	if (isset($_GET['keywords']) && isset($_GET['searchMethod'])) {
		if ($_GET['searchMethod'] == "category") {
			$categoryName = $_GET['keywords'];
			$categoryInfoGrabber = mysql_query("SELECT * FROM `modulecategories` WHERE `organization` LIKE '%{$categoryName}%'", $connDBA);
			
			if ($categoryInfo = mysql_fetch_array($categoryInfoGrabber)) {
				$keywords = $categoryInfo['id'];
		...
		formErrors();
			...
			<tbody>
				<tr>";
					...
					if (isset ($_GET['sort']) && isset ($_GET['order']) && $_GET['sort'] == "type.category" && $_GET['order'] == "descending.ascending") {
						echo "?" . $additionalParameters . "sort=type.category&order=ascending.ascending\" class=\"ascending\">Type</a>";
					} elseif (isset ($_GET['sort']) && isset ($_GET['order']) && $_GET['sort'] == "type.category" && $_GET['order'] == "ascending.ascending") {
						echo "?" . $additionalParameters . "sort=type.category&order=descending.ascending\" class=\"descending\">Type</a>";
					} elseif (!isset ($_GET['sort'])) {
						echo "?" . $additionalParameters . "sort=type.category&order=descending.ascending\" class=\"descending\">Type</a>";
					} else {
						echo "?" . $additionalParameters . "sort=type.category&order=ascending.ascending\" class=\"sortHover\">Type</a>";
					}
					echo "</th>
					<th width=\"150\" class=\"tableHeader\"><a href=\"search.php";
					if (isset ($_GET['sort']) && isset ($_GET['order']) && $_GET['sort'] == "category.type" && $_GET['order'] == "descending.ascending") {
						echo "?" . $additionalParameters . "sort=category.type&order=ascending.ascending\" class=\"ascending\">Category</a>";
					} elseif (isset ($_GET['sort']) && isset ($_GET['order']) && $_GET['sort'] == "category.type" && $_GET['order'] == "ascending.ascending") {
						echo "?" . $additionalParameters . "sort=category.type&order=descending.ascending\" class=\"descending\">Category</a>";
					} else {
						echo "?" . $additionalParameters . "sort=category.type&order=ascending.ascending\" class=\"sortHover\">Category</a>";
					}
					echo "</th>
			...
			while(($questionData = mysql_fetch_array($questionGrabber)) && ($number <= $questionNumber)) {
				...
				if ($_GET['searchMethod'] == "category") {
					echo str_ireplace($_GET['keywords'], "<span class=\"searchKeywords\">" . strtolower($_GET['keywords']) . "</span>", commentTrim(25, $questionData['category']));
				} else {
					echo commentTrim(25, $questionData['category']);
				}
