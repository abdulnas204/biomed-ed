...
//Process the form	
	if (isset($_POST['submit']) && !empty($_POST['name']) && !empty($_POST['description']) && is_numeric($_POST['require']) && is_numeric($_POST['testFilter']) && is_numeric($_POST['autoSuggest']) && is_numeric($_POST['showTip']) && !empty($_POST['fieldType']) && !empty($_POST['section'])) {
		...
		$selected = escape(arrayStore($_POST['selected']));
		$values = escape(arrayStore($_POST['values']));
		$section = escape(arrayStore($_POST['section']));
...
//Fields form
	...
	if (isset($fields)) {
		$selected = arrayRevert($fields['selected']);
	...
	if (isset($fields)) {
		if (in_array("Lesson Settings", arrayRevert($fields['section']))) {
			$firstValue = true;
		}
		
		if (in_array("Question Generator", arrayRevert($fields['section']))) {
			$secondValue = true;
		}
	}
