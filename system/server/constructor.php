...
This script contains functions used to create common HTML elements, and add a touch of dynamic content, while abiding to system-wide HTML standards.
*/
...
//Directions for a form element
	function directions($text, $required = false, $help = false) {
		...
		if ($help == true) {
			echo "\n<img src=\"" . $root . "system/images/admin_icons/help.png\" alt=\"Help\" width=\"17\" height=\"17\" onmouseover=\"Tip('" . addslashes($help) . "')\" onmouseout=\"UnTip()\" />";
		}
		...
		switch ($type) {
			case "Custom Content" :				
				if (!loggedIn()) {
					...
					echo "&nbsp;" . URL("", "cms/manage_sidebar.php?id=" . $editID, "smallEdit") . "</h2>\n<div>\n" . $text . "\n</div>\n";
				} else {
					echo "</h2>\n<div>\n" . $text . "\n</div>\n";
				}
...
//Live action, for hiding/showing an element, or checking/unchecking a fake checkbox
	function option($table, $id, $type = "visible", $width = "25", $requiredPrivilege = false) {
		...
		if ($doAction == true) {
			...
			$return .= "<div align=\"center\">\n";
			$return .= URL("", "javascript:;", $type . $class, false, false, false, false, false, false, " id=\"" . $id . "\"");
			$return .= "\n</div>\n";
			$return .= "</td>\n";
...
//Create a regular table cell
	function cell($content, $width = false, $requiredPrivilege = false) {
		...
		if ($doAction == true) {
			return "<td" . $cellWidth . " valign=\"top\">" . $content . "</td>\n";
		}
	}
