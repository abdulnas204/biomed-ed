...
This script is used to process, maintain, and secure all login actions and user-related queries.
*/
	...
	function maintain() {
		...
		if (isset($privileges)) {
			if (loggedIn()) {
				...
				foreach(arrayRevert($allPrivileges['privileges']) as $value) {
...
//Check the user's access to a particular item
	function access() {
		if (loggedIn()) {
			...
			$privileges = arrayRevert($role['privileges']);
			$return = false;
