...
//Process the form
	if (isset($_POST['submit']) && !empty($_POST['firstName']) && !empty($_POST['lastName']) && !empty($_POST['userName']) && !empty($_POST['primaryEmail'])) {
		if (!exist("user", "userName", mysql_real_escape_string(preg_replace("/[^a-zA-Z0-9\s]/", "", $_POST['userName'])))) {
...
//Title
	if (!isset($_GET['error'])) {
		...
		title("Register", "Begin here to set up an account to enroll in our training program.", false);
	}
...
//Registration form
	...
	echo "<blockquote><p>";
    textField("userName", "userName", false, false, false, true, ",length[6,30],custom[noSpecialCharactersSpaces],ajax[ajaxName]");
	echo "</p></blockquote>";
