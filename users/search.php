	...
	if (!isset($_GET['keywords']) && !isset($_GET['searchMethod']) && !isset($_GET['suggestions'])) {
		...
		formErrors();
			...
			<tbody>
				<tr>";
					...
					if (isset ($_GET['sort']) && isset ($_GET['order']) && $_GET['sort'] == "organization.lastName" && $_GET['order'] == "ascending.ascending") {
						echo "?" . $additionalParameters . "sort=organization.lastName&order=descending.ascending\" class=\"descending\">Assigned Organization</a>";
					} elseif (isset ($_GET['sort']) && isset ($_GET['order']) && $_GET['sort'] == "organization.lastName" && $_GET['order'] == "descending.ascending") {
						echo "?" . $additionalParameters . "sort=organization.lastName&order=ascending.ascending\" class=\"ascending\">Assigned Organization</a>";
					} else {
						echo "?" . $additionalParameters . "sort=organization.lastName&order=ascending.ascending\" class=\"sortHover\">Assigned Organization</a>";
					}
					echo "</th>
			...
			while(($userData = mysql_fetch_array($userGrabber)) && ($number <= $userNumber)) {
				...
				echo "</a></td>" . 
				"<td width=\"150\"><a href=\"../communication/send_email.php?type=user&id=" . $userData['id'] . "\">";
