...
//Process the form
	...
	//If the page is updating an item
		...
		//If a new file is not uploaded	
			...
			header ("Location: ../index.php?category=" . $_SESSION['bankCategory'] . "&updated=file");
			exit;
			...
			if ($questionBankCheck = mysql_fetch_array($questionBankCheckGrabber)) {
				...
				while ($questionBankInsert = mysql_fetch_array($questionBankInsertGrabber)) {
					if ($questionBankInsert['questionBank'] == "1") {
						...
						header ("Location: ../index.php?category=" . $_SESSION['bankCategory'] . "&inserted=file&export=true&exportID=" . $linkID);
						exit;
					}
				}
			} else {
				header ("Location: ../index.php?category=" . $_SESSION['bankCategory'] . "&inserted=file");
				exit;
			}
		}
	}
