...
//Process the form
	...
	//If the page is updating an item
		...
		//Get form data values
			...
			header ("Location: ../index.php?id=" . $_SESSION['bankCategory'] . "&updated=choice");
			exit;
	//If the page is inserting an item		
		...
		//Get form data values
			...
			$insertChoiceQuery = "INSERT INTO questionbank (
							...
							NULL, 'Multiple Choice', '{$points}', '{$extraCredit}', '{$partialCredit}', '{$difficulty}', '{$category}', '{$link}', '{$randomize}', '0', '{$interface}', '1', '{$tags}', '{$question}', '{$questionValue}', '', '{$answerValue}', '', '{$feedBackCorrect}', '{$feedBackIncorrect}', '{$feedBackPartial}'
							)";
		...
		//Automatically insert this question into tests of a similar category
			...
			if ($questionBankCheck = mysql_fetch_array($questionBankCheckGrabber)) {
				...
				header ("Location: ../index.php?id=" . $_SESSION['bankCategory'] . "&inserted=choice&export=true&exportID=" . $linkID);
				exit;
			} else {
				header ("Location: ../index.php?id=" . $_SESSION['bankCategory'] . "&inserted=choice");
				exit;
			}
		}
	}
