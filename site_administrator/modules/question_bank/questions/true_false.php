...
//Process the form
	if (isset ($_POST['submit']) && !empty($_POST['question']) && is_numeric($_POST['points']) && !empty($_POST['answer']) && !empty($_POST['category'])) {
		...
		//Get form data values
			...
			header ("Location: ../index.php?category=" . $_SESSION['bankCategory'] . "&updated=truefalse");
			exit;
			...
			if ($questionBankCheck = mysql_fetch_array($questionBankCheckGrabber)) {
				...
				while ($questionBankInsert = mysql_fetch_array($questionBankInsertGrabber)) {
					if ($questionBankInsert['questionBank'] == "1") {
						...
						header ("Location: ../index.php?category=" . $_SESSION['bankCategory'] . "&inserted=truefalse&export=true&exportID=" . $linkID);
						exit;
					}
				}
			} else {
				header ("Location: ../index.php?category=" . $_SESSION['bankCategory'] . "&inserted=truefalse");
				exit;
			}
		}
	}
