...
//Process the form
	if (isset($_POST['submit']) && !empty ($_POST['title']) && !empty($_POST['toDetirmine']) && !empty($_POST['toImport']) && !empty($_POST['content'])) {	
		...
		//Ensure times are not inferior if the dates are the same
			...
			if (!empty($fromDate)) {
				$editAnnouncementQuery = "UPDATE announcements SET `visible` = 'on', `display` = '{$toDetirmine}', `to` = '{$toImport}', `fromDate` = '{$fromDate}', `fromTime` = '{$fromTime}', `toDate` = '{$toDate}', `toTime` = '{$toTime}', `title` = '{$title}', `content` = '{$content}' WHERE `id` = '{$announcement}'";
			} else {
...
<p>To<?php if (isset($_GET['type'])) {if ($_GET['type'] == "users" || $_GET['type'] == "organizations" || $_GET['type'] == "roles") {echo "<span class=\"require\">*</span>";}} ?>:</p>
              ...
              switch ($_GET['type']) {
                  ...
                  case "roles" : 
					  ...
					  if (isset($announcement)) {
						  ...
						  echo "<option value=\"Administrative Assistant\">Administrative Assistant</option><option value=\"Instructor\">Instructor</option><option value=\"Instructorial Assisstant\">Instructorial Assisstant</option><option value=\"Organization Administrator\">Organization Administrator</option><option value=\"Site Administrator\">Site Administrator</option><option value=\"Site Manager\">Site Manager</option><option value=\"Student\">Student</option>";
					  }
        ...
        <blockquote>
          <p>
            ...
            <option value="12:00"<?php if (isset ($announcement) && $announcement['fromTime'] == "12:00") {echo " selected=\"selected\"";} elseif (!isset ($announcement)) {echo " selected=\"selected\"";} ?>>12:00 pm</option>
          ...
          <select name="toTime" id="toTime"<?php if (isset ($announcement) && $announcement['toTime'] == "") {echo " disabled=\"disabled\"";} elseif (!isset($announcement)) {echo " disabled=\"disabled\"";} ?>>
            ...
            <option value="13:00"<?php if (isset ($announcement) && $announcement['toTime'] == "13:00") {echo " selected=\"selected\"";} elseif (!isset ($announcement)) {echo " selected=\"selected\"";} ?>>1:00 pm</option>
